...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        print("BartEncoder: SinusoidalPositionalEmbedding(config.max_position_embeddings, embed_dim, self.padding_idx)")
        ...
        print("nn.Embedding(1460, embed_dim)")
        ...
        print("self.layernorm_embedding = LayerNorm(embed_dim) if config.normalize_embedding else nn.Identity()")
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        print("BartDecoder: self.embed_positions = LearnedPositionalEmbedding(config.max_position_embeddings, config.d_model, self.padding_idx)")
        # if config.static_position_embeddings:
        #     self.embed_positions = SinusoidalPositionalEmbedding(
...
class ICDBartForSequenceClassification(PretrainedBartModel):
    def __init__(self, config: BartConfig, **kwargs):
        ...
        if hasattr(config, "loss_weights"):
            if (config.loss_weights is not None):
                self.loss_weight = torch.tensor(config.loss_weights).cuda()
...
class ICDBartForSequenceBinaryClassification(PretrainedBartModel):
    def __init__(self, config: BartConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.num_labels = config.num_labels
        ...
        self.model._init_weights(self.classification_head.dense)
        self.model._init_weights(self.classification_head.out_proj)
        if hasattr(config, "neg_weight") and hasattr(config, "pos_weight"):
            if (config.neg_weight is not None) and (config.neg_weight is not None):
                self.loss_weight = torch.tensor([config.neg_weight, config.pos_weight]).cuda()
        else:
            self.loss_weight = None
    ...
    Examples::
        ...
        outputs = (logits,) + outputs[1:]  # Add hidden states and attention if they are here
        if labels is not None:  # prepend loss to output,
            loss_weight = torch.unsqueeze(labels,1).float()*self.loss_weight[1] + (1-torch.unsqueeze(labels,1).float())*self.loss_weight[0]
            loss = F.binary_cross_entropy(F.sigmoid(logits), torch.unsqueeze(labels,1).float(), weight=loss_weight )
            outputs = (loss,) + outputs
...
class SinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def _init_weight(out: nn.Parameter):
        ...
        # out[:, 0 : dim // 2] = torch.sin(torch.tensor(position_enc[:, 0::2]), requires_grad=False)
        # out[:, 0 : dim // 2] = torch.tensor(position_enc[:, 0::2]).sin().detach()
